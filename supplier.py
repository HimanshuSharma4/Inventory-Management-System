...
class supplierClass:
    def __init__(self,root):
        ...
        self.valid_invoice = (self.root.register(self.validate_invoice), '%P')
        ...
        txt_supplier_invoice=Entry(self.root,textvariable=self.var_sup_invoice,font=("goudy old style ",15),bg="lightyellow",validate="key", validatecommand=self.valid_invoice).place(x=180,y=80,width=180)
       
       
        #row 2
        lbl_name=Label(self.root,text="Name",font=("goudy old style ",15),bg="white").place(x=50,y=120)
        txt_name=Entry(self.root,textvariable=self.var_name,font=("goudy old style ",15),bg="lightyellow",validate="key", validatecommand=self.valid_text).place(x=180,y=120,width=180)
        

        #row 3
        lbl_contact=Label(self.root,text="Contact",font=("goudy old style ",15),bg="white").place(x=50,y=160)
        txt_contact=Entry(self.root,textvariable=self.var_contact,font=("goudy old style ",15),bg="lightyellow",validate="key", validatecommand=self.valid_contact).place(x=180,y=160,width=180)
...
#===================================================================
    def delete(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                # Pehle check karein ki invoice exist karta hai ya nahi
                cur.execute("Select * from supplier where invoice=?",(self.var_sup_invoice.get(),))
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid Invoice No.",parent=self.root)
    ...
    def validate_text(self, P):
        # Allows only letters and spaces (For Name)
        if P == "":
            return True
        for char in P:
    ...
    def validate_invoice(self, P):
        # Allows strictly numbers (digits) for the Invoice No.
        if P == "":
            return True
        if P.isdigit():
            return True
